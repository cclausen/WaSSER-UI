...
export class PlacesDatasource extends DataSource<Place> {
  ...
  constructor(private placesApi: PlaceControllerService) {
    super();
  }
  ...
  public index(): void {
    this.placesApi.indexPlaces().subscribe((data) => {
      this.data = data;
      this.trigger.next();
    });
  }
   ...
   */
  private getPagedData(data: Place[]): Place[] {
    if (this.paginator) {
   ...
   * this would be replaced by requesting the appropriate data from the server.
   */
  private getSortedData(data: Place[]): Place[] {
